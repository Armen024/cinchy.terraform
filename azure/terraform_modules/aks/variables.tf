...
variable "vm_size" {
  ...
  description = "The default virtual machine size for the Kubernetes node"
  type        = string
}
...
variable "node_count" {
  description = "The number of node that should exist in the node Pool. Please set `node_count` `null` while `enable_auto_scaling` is `true` to avoid possible `node_count` changes."
  type        = number
  default     = 2
}
...
variable "node_pool_name" {
  description = "The default Azure AKS nodepool (nodepool) name."
  type        = string
  default     = "nodepool"
}
...
variable "node_max_pods" {
  description = "(Optional) The maximum number of pods that can run on each node. Changing this forces a new resource to be created."
  type        = number
  default     = 250
}
...
variable "private_dns_name" {
  description = "Managed Kubernetes Cluster DNS Name."
  type        = string
  default     = null
}