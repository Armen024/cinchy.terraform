...
#   # value       = try(split("//", aws_eks_cluster.this[0].identity[0].oidc[0].issuer[1], ""))
#   value       = try(split("//", aws_eks_cluster.this[0].cluster_oidc_issuer_url)[1], "EKS Cluster not enabled") # TODO - remove `split()` since `oidc_provider` coverss https:// removal
